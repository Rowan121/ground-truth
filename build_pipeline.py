...
instructions = [
 ...
 'Recall related prior evidence by calling evidence.recall (tool node evidence). An empty memory is acceptable; a failed tool must be reported.',
 'Call sources.http_request (tool node sources) using GET to search https://api.crossref.org/works with query.bibliographic and rows=5, and https://www.ebi.ac.uk/europepmc/webservices/rest/search with query, format=json and pageSize=5. Retrieve abstracts or accessible full text and follow explicit references where available. Limit the first run to 5 sources.',
 ...
 'Send retrieved evidence with its source identifiers to evidence.remember. Wait for evidence.memory_status completed before calling evidence.recall. Ask Cognee to extract supported claims and contradictions with references. Never invent missing quotations.',
 'Load normalized source claim records into analytics.load_data, then call analytics.get_data to compare quantities, units, populations and outcomes. Record actual query results. Do not fabricate counts.',
